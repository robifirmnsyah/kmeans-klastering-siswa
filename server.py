...
def dashboard():
    ...
    # Ambil jurusan yang dipilih dari query string
    selected_jurusan = request.args.get('jurusan', default='Rekayasa Perangkat Lunak')

    # Query untuk total siswa berdasarkan jurusan yang dipilih
    cursor.execute("""
    ...
    # Query untuk jumlah siswa per cluster berdasarkan jurusan yang dipilih
    cursor.execute("""
        SELECT COUNT(*) AS jumlah 
        FROM data_siswa 
        WHERE cluster = 1 AND jurusan = %s
    """, (selected_jurusan,))
    ...
    for tahun in years:
        cursor.execute("""
            ...
            FROM data_siswa
            WHERE tahun_ajaran = %s AND jurusan = %s
            GROUP BY cluster
        """, (tahun, selected_jurusan))
    ...
    if request.method == 'POST':
        ...
        if 'simpan' in request.form:
            ...
            try:
                ...
                for baris in preview_data:
                    sql = """INSERT INTO data_siswa (nip, nis, nama_siswa, kelas, mapel1, mapel2, mapel3, 
...
def delete_hasil_analisis():
    ...
    cursor = db.cursor()
    try:
        sql = """DELETE FROM data_siswa 
                 WHERE nip = %s 
                 AND kelas = %s 
                 AND jurusan = %s 
                 AND semester = %s 
                 AND tahun_ajaran = %s"""
        val = (nip, kelas, jurusan, semester, tahun_ajaran)
        cursor.execute(sql, val)
        db.commit()
...
def hasil_klastering():
    ...
    cursor = db.cursor(dictionary=True)
    sql = '''SELECT * FROM data_siswa WHERE kelas = %s AND jurusan = %s AND semester = %s AND tahun_ajaran = %s'''
